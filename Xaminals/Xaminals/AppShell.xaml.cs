...
namespace Xaminals
{
    public partial class AppShell : Shell
    {
        ...
        async Task NavigateToRandomPageAsync()
        {
            ...
            var shellItems = Shell.Current.Items.ToList();
            var tabs = shellItems.FirstOrDefault(s => s.Title == "Animals");
            try
            {
                //Expected behaviour: the bottom tabs are removed from the shellItem
                tabs.Items.Clear();
            }
            catch (Exception e)
            {
                //Actual behaviour: an exception thrown because the application tries to set properties on the tab bar, which no longer exists
                var ex = e;
            }
